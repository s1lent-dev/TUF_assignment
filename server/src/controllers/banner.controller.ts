...
const toggleBannerStatus = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    await pool.query("UPDATE banners SET isVisible=? WHERE id=?", [status, id]);
    redis.del("banners");
...
const addBanner = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    await pool.query("INSERT INTO banners (title, description, link, timer) VALUES (?, ?, ?, ?)", [title, description, link, timer]);
    redis.del("banners");
...
const updateBannerDescription = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    await pool.query("UPDATE banners SET description=? WHERE id=?", [
      description,
      id,
    ]);
    redis.del("banners");
...
const updateBannerTimer = asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
    ...
    await pool.query("UPDATE banners SET timer=? WHERE id=?", [timer, id]);
    redis.del("banners");
...
const updateBannerLink = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const { link } = req.body;
    await pool.query("UPDATE banners SET link=? WHERE id=?", [link, id]);
    redis.del("banners");
...
export { getBanners, toggleBannerStatus, addBanner, updateBannerDescription, updateBannerTimer, updateBannerLink };